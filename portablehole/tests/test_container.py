...
class TestContainer(unittest.TestCase):
    ...
    def _build_container():
        ...
        new_con = portablehole.items.Container(name='Bag', capacity=200.0, weight=10.0, value=15.0)
    ...
    def test_creation(self):
        ...
        assert new_con.category == portablehole.items.ItemCategory.CONTAINER, 'Category is incorrect.'
        assert new_con.id == f'44756014165|{portablehole.items.ItemCategory.CONTAINER}|Bag', (
    ...
    def test_full(self):
        ...
        fill = portablehole.items.Collection(
            item=portablehole.items.Item(name='fill', weight=200.0 - 30.0, value=1.0))
        new_con.add(fill)
    ...
    def test_add(self):
        ...
        add_fill = portablehole.items.Collection(item=add_item, quantity=200)
        con_id = add_con.id
        ...
        assert f'{portablehole.items.ItemCategory.MISC}|test' in new_con.contents, (
            'Collection was not added.'
        )
        assert new_con.contents[f'{portablehole.items.ItemCategory.MISC}|test'].quantity == 1, (
            'Collection was not copied to container.'
        )
        ...
        assert new_con.contents[f'{portablehole.items.ItemCategory.MISC}|test'].quantity == 7, (
            'Same collections are not cumulated.'
        )

        full = new_con.add(add_fill)
        assert isinstance(full, tuple) and len(full) == 1, (
            'Full container should return tuple of items not added.'
        )
    ...
    def test_sub(self):
        ...
        test = new_con.sub(f'{portablehole.items.ItemCategory.WEAPON}|Sword')

        assert f'{portablehole.items.ItemCategory.WEAPON}|Sword' not in new_con.contents, (
            'Collection was not subtracted.'
        )
        assert test, 'sub method should return true if collection was subtracted.'

        test = new_con.sub(f'{portablehole.items.ItemCategory.MISC}|test')
